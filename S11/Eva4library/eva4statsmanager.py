...
class StatsManager:
  def __init__(self):
    self.data={"mis_classified_images":[],"pred":[],"target":[],"train_loss":[],"test_loss":[],"train_accuracy":[],"test_accuracy":[],'lr':[]}
  ...
  def plot_losses(self):
    ...
    plt.show()
  def plot_lr_accuracies(self):
  	max_ac=max(self.data["train_accuracy"])
  	max_lr=self.data["lr"][self.data["train_accuracy"].index(max_ac)]
  	y=np.arange(0,max_ac,10)
  	x=np.zeros_like(y)
  	plt.plot(self.data["lr"],self.data["train_accuracy"],"-g")
  	plt.plot(x+max_lr,y,'--r')
  	plt.xlabel("lr")
  	plt.ylabel("accuracy")
  	plt.show()
  
  def give_max_lr(self):
  	max_ac=max(self.data["train_accuracy"])
  	return self.data["lr"][self.data["train_accuracy"].index(max_ac)]
